<template>
  <div id="root-container">
    <div id="main-container" @mouseenter="hasFocus = true" @mouseleave="hasFocus = false" @focus="hasFocus = true" @blur="hasFocus = false">
      <label v-show="!editing" focusable="false" class="name-label" @click="isSelected = !isSelected" @keyup.enter="isSelected = !isSelected" >{{source.name}}</label>
      ...
      <span id="buttons-container">
        ...
        <a href="#" v-hide="!hasFocus" @click="handleMarkerClick" class="icon-link"><font-awesome-icon class="icon" icon="map-marker-alt"/></a>
        <a href="#" v-hide="!hasFocus" @click="handleEditClick" class="icon-link"><font-awesome-icon :class="['icon', { 'icon-active' : editing} ]" icon="pencil-alt"/></a>
      </span>
    </div>
    <transition-expand>
      <div v-if="isSelected" class="detail-container">
        <div class="detail-row"><span class="prompt">RA:</span><span>{{ this.raStr }}</span></div>
        <div class="detail-row"><span class="prompt">Dec:</span><span>{{ this.decStr }}</span></div>
        <div class="detail-row"><span class="prompt">Catalog:</span><span>{{ this.source.catalogName }}</span></div>
        <div class="detail-row"><span class="prompt">Query Coordinates:</span><a :href="this.simbadCoordinatesURL" target="_blank" rel="noopener noreferrer">SIMBAD</a> | 
        <a :href="this.nedCoordinatesURL" target="_blank" rel="noopener noreferrer">NED</a></div>
      </div>
    </transition-expand>
  </div>
</template>
...
export default class SourceItem extends Vue {
    ...
    editingChanged(val: boolean, _oldVal: boolean) {
      const input: HTMLInputElement = this.$el.getElementsByClassName('name-input')[0] as HTMLInputElement;
      if (val) {
